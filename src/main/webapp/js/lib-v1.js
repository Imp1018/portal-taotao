...
var category = {
   ...
   // URL_Serv: "http://manage.taotao.com/web/itemcat/all?callback=category.getDataService",
    URL_Serv: "http://172.16.30.2:8081/cat/all?callback=category.getDataService",
    ...
    FN_GetLink: function (a, b) {
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http:172.16.30.2:8081" + c ), '<a href="' + c + '">' + d + "</a>")
    },
